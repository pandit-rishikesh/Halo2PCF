...
//	double *vx=dvector(0,n);
	...
	ifstream in (string("/storage/DATA-03/astrorm3/Users/rpandit/sim/zHorizon/gc/anukruti/z_0.5/41-200/run"+number+".dat").c_str());
	assert(in);
	
	for (long i=0; i<=n; i++) {xd[i]=0;yd[i]=0;zd[i]=0;} 
	for (long i=1; i<=n; i++) in >> xd[i]>> yd[i] >> zd[i] >> aux >> aux >> aux >> aux;	
	in.close();
...
//======================================= Writing output =========================================//
	...
	ofstream ddr(string("/storage/DATA-03/astrorm3/Users/rpandit/output/2pcf/real/z_0.5/pairs/41-200/ddrrdr_r_50x50_0_200Mpc_run"+number+".dat").c_str());
...
/*	ofstream ddr2(string("/storage/DATA-03/astrorm3/Users/rpandit/output/2pcf/red/z_0.5/pairs/41-200/ddrrdr_s_mu_12x50_50_200Mpc_run"+number+".dat").c_str());
	for(int i=1; i<=sbins; i++)
	...
	ddr2.close();
*/
	ofstream ddr3(string("/storage/DATA-03/astrorm3/Users/rpandit/output/2pcf/real/z_0.5/pairs/41-200/ddrrdr_rpar_rperp_50x50_0_200Mpc_run"+number+".dat").c_str());
...
//============================================ Writing LogFiles ================================//
	...
	ofstream logfile(string("/storage/DATA-03/astrorm3/Users/rpandit/logfiles/realcorr_z_0.5_41-200"+logdate+".log").c_str());

	logfile <<"Log File with parameters for real space pair calculator\n\n\n";
	logfile <<"We are at redshift z = :"<< Z << "\n";
	logfile <<"No. of rbins :"  << "\t" << Nbins << "\n";
